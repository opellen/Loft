...
> 노트에 이미지를 붙여넣거나 드래그하면 Loft가 본인의 Google Drive에 업로드하고, 렌더링되는 `![](링크)`를 삽입합니다 — vault를 가볍게 유지하면서.
...
- **붙여넣기/드래그 업로드.** 이미지를 노트에 붙여넣거나 드래그하면 본인의 Google Drive로 자동 업로드되고, 커서 위치에 렌더링되는 `![](링크)`가 삽입됩니다.
